...
void perm(int list[], int k, int m, int* p, Mat* mat, float* det) {
  ...
  if (k > m) {
    ...
    for (i = k + 1; i <= m; i++) {
      if (equal(mat->element[k][list[i]], 0.0f)) continue;
      swap(&list[k], &list[i]);
      *p += 1;
      perm(list, k + 1, m, p, mat, det);
      swap(&list[k], &list[i]);
...
Mat* MatCreate(Mat* mat, int row, int col) {
  ...
  for (i = 0; i < row; i++) {
    ...
    if (mat->element[i] == NULL) {
      int j;
      printf("mat create fail!\n");
      for (j = 0; j < i; j++) free(mat->element[j]);
      free(mat->element);
      return NULL;
    }
  }
...
void MatDelete(Mat* mat) {
  ...
  for (i = 0; i < mat->row; i++) free(mat->element[i]);
  free(mat->element);
}
